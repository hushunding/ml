...
def load_data_wrapper2(path):
    train_images, train_labels, test_images, test_labels = mnist_web.mnist(path)
...
def vectorized_result2(y):
    """Return a 10-dimensional unit vector with a 1.0 in the jth
    position and zeroes elsewhere. This is used to convert a digit
    (0...9) into a corresponding desired output from the neural
    network."""    
    e = np.reshape([ 1.0 if j else 0.0  for j in y], (10,1))
    return e

def vectorized_result3(y):
    e = y.tolist().index(True)
    return e
